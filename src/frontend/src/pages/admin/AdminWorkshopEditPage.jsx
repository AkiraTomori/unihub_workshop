...
export default function AdminWorkshopEditPage({ token, onToast, onWorkshopsChanged }) {
  ...
  const [aiSummaryStatus, setAiSummaryStatus] = useState("PENDING");
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        // Fetch AI Summary from document
        const document = await api.getDocument(token, id).catch(() => null);
        if (document) {
          setAiSummary(document.aiSummary || "");
          setAiSummaryStatus(document.processStatus || "PENDING");
        }
      ...
      {loading ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <label className="block text-sm text-blue-900">
            ...
            <div className="mt-1 flex items-center gap-2 text-xs text-blue-600 mb-1">
              <span>Status:</span>
              <span className={`font-medium px-2 py-1 rounded ${aiSummaryStatus === 'COMPLETED' ? 'bg-green-100 text-green-700' : aiSummaryStatus === 'PROCESSING' ? 'bg-yellow-100 text-yellow-700' : aiSummaryStatus === 'FAILED' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-700'}`}>
                {aiSummaryStatus}
              </span>
            </div>
            <textarea
              className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 bg-gray-50 text-gray-700 cursor-not-allowed"
              rows={5}
              value={aiSummary}
              readOnly
            ...
            <p className="mt-2 text-xs text-gray-600">
              💡 This field is auto-generated by AI and cannot be edited directly.
            </p>
          </label>
