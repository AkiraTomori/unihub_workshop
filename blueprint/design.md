...
The system uses a **Circuit Breaker** combined with **Graceful Degradation** and **Exponential Backoff with Jitter** for temporary gateway errors (HTTP 5xx). This protects the main registration flow from being blocked by an external payment outage.

#### Gateway retry (before opening the circuit)

For **retriable 5xx** responses only, the backend retries the provider call with exponential backoff and jitter:

| Attempt | Backoff base | Actual delay |
|---|---|---|
| 1st retry | 100 ms | 100 ms + random jitter (0–100 ms) |
| 2nd retry | 200 ms | 200 ms + random jitter (0–200 ms) |
| 3rd retry | 400 ms | 400 ms + random jitter (0–400 ms) |

Non-retriable errors (for example client rejection or timeout simulation) are not retried with this backoff policy.

If all retries are exhausted on 5xx, the circuit breaker opens.

#### Circuit breaker states

| Circuit State | Trigger | Behavior |
|---|---|---|
| Closed | Normal operation | Requests flow to the payment provider (with 5xx retry policy above) |
| Open | All 5xx retries exhausted for a checkout attempt | Stop sending new payment requests to the external gateway for the cooldown period |
| Half-Open | After **30 seconds** cooldown | Allow up to **3** probe requests to test recovery |

| Half-Open outcome | Next state |
|---|---|
| Probe succeeds | **Closed** — normal traffic resumes |
| Probe fails | **Open** — another 30 second cooldown |

Configurable via environment variables: `CIRCUIT_BREAKER_OPEN_DURATION_MS` (default `30000`), `CIRCUIT_BREAKER_HALF_OPEN_PROBES` (default `3`), `PAYMENT_RETRY_DELAYS_MS` (default `100,200,400`).

When the circuit is open, the system does not stop browsing or registration completely. Instead, checkout returns `PENDING_PAYMENT` and the student can retry later from **My Payments** or the registration payment dialog. This keeps the rest of the application usable even when the payment partner is down.
...
| Return `PENDING_PAYMENT` with clear message (including retry hint when circuit is open) | Student knows payment must be retried later |
...
|---|---|---|
| POST | `/api/payments/checkout` | Student checkout (primary UI path) |
| POST | `/api/payments` | Alias for checkout |
| POST | `/api/payments/webhook` | Signed gateway callback (`x-webhook-secret`) |

### Double-Charge Prevention
To prevent duplicate payment processing, the system uses an Idempotency Key mechanism backed by Redis and PostgreSQL.

| Item | Design Choice |
|---|---|
| Key Source | UUID generated by the client when the payment session starts; reused on retry for the same checkout |
| Redis storage | Short in-flight lock (`PROCESSING`, 60 s) + terminal success cache (`CONFIRMED`, 24 h TTL) |
| PostgreSQL storage | `payments.idempotency_key` as durable record of processed checkout |
| TTL | 24 hours for successful replay cache (`PAYMENT_IDEMPOTENCY_TTL_SECONDS`) |
| Purpose | Prevent the same payment request from being processed more than once |

Processing flow:

1. The client generates an idempotency key when starting payment and keeps the same key for retries of that checkout.
2. The backend checks Redis for a cached **terminal success** result; if found, it replays that response.
3. The backend reserves a short-lived Redis lock while processing.
4. If the circuit is open, the backend returns `PENDING_PAYMENT` without caching a terminal failure (so the student can retry after cooldown).
5. On gateway success, the backend writes `SUCCESS` to PostgreSQL, confirms the registration, enqueues the confirmation email, and caches the `CONFIRMED` result in Redis.
6. On gateway failure after retries, the backend stores or updates a `PENDING` payment row, releases the Redis lock, and returns `PENDING_PAYMENT` so the client can retry with the same key.
7. If the same successful request is retried, the backend returns the previous `CONFIRMED` result and skips duplicate processing.

Pending and failed checkout outcomes are **not** cached as terminal idempotency results in Redis, so students are not stuck replaying a stale pending response.
